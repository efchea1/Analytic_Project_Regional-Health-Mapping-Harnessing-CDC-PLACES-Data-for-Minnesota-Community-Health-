...
server <- function(input, output, session) {
  ...
  output$selected_county_title <- renderText({
    HTML(paste(input$parGlobal_county, "County", "<br/>Coronary Heart Disease Exposure")) # Create the name of the selected county above the graph
  })
  ...
  output$selected_region_title <- renderText({
    ...
    HTML(paste(county_region, "Region", "<br/>Coronary Heart Disease Exposure")) # Create the name of the selected region above the graph
  })
  
  output$selected_state_title <- renderText({
    HTML(paste("Minnesota", "<br/>Coronary Heart Disease Exposure")) # Create the name of the state above the graph
  })
  ...
  output$selected_chb_title <- renderText({
    ...
    HTML(paste(county_chb, "CHB", "<br/>Coronary Heart Disease Exposure")) # Create the name of the selected CHB above the graph
  })
...
# #     summarise(Aggregate_Data_Value = sum(Aggregate_Data_Value) / sum(AGE18PLUS_TOT) * 100, # Calculate aggregate data value
# #               Aggregate_Low_Confidence_Limit = sum(Aggregate_Low_Confidence_Limit) / sum(AGE18PLUS_TOT) * 100, # Calculate aggregate low confidence limit
# #               Aggregate_High_Confidence_Limit = sum(Aggregate_High_Confidence_Limit) / sum(AGE18PLUS_TOT) * 100, .groups = 'drop') # Calculate aggregate high confidence limit
# # }
# # 
# # # # Calculate crude values
# # # CHD_Crude21MN <- aggregate_values(PopEst_CHDMN |> filter(Data_Value_Type == 'Crude prevalence'), 'Region', 'Region')
# # # # Calculate age-adjusted values
# # # CHD_Adj21MN <- aggregate_values(PopEst_CHDMN |> filter(Data_Value_Type == 'Age-adjusted prevalence'), 'Region', 'Region')
...
# #   }) # Filter and aggregate CHD data for the selected county
# #   
# #   output$plot_confidenceInterval <- renderPlot({
# #     data <- reactive_CHD_data() # Get the filtered CHD data
# #     ggplot(data, aes(x = Data_Value_Type, y = Aggregate_Data_Value, color = Data_Value_Type)) +
...
# #   }) # Filter and aggregate CHD data for the selected region
# #   
# #   output$plot_chbRegion <- renderPlot({
# #     data <- reactive_region_data() # Get the aggregated CHD data for the region
# #     ggplot(data, aes(x = Data_Value_Type, y = Aggregate_Data_Value, color = Data_Value_Type)) +
...
# #   }) # Filter and aggregate CHD data for the selected CHB
# #   
# #   output$plot_chdCHB <- renderPlot({
# #     data <- reactive_chb_data() # Get the aggregated CHD data for the CHB
# #     ggplot(data, aes(x = Data_Value_Type, y = Aggregate_Data_Value, color = Data_Value_Type)) +
...
#     selectInput("par_chdStateRegionChb", label = "Select Comparison", choices = c("All", "State", "Region", "Community Health Board"), selected = "All", multiple = FALSE, width = 350),
#     sidebarMenu(
#       menuItem("Home", tabName = "tn_homePage"),
#       menuItem("Region & CHB Definition", tabName = "tn_regionChbDefinitions"),
#       menuItem("Coronary Heart Disease", tabName = "tn_coronaryHeartDisease")
...
#   
#   output$selected_state_title <- renderText({
#     HTML(paste("Minnesota State", "<br/>Coronary Heart Disease Exposure"))
#   })
#   
#   output$selected_chb_title <- renderText({
#     HTML(paste(input$parGlobal_county, "County", "<br/>Coronary Heart Disease Exposure"))
...
#       geom_errorbar(aes(ymin = Aggregate_Low_Confidence_Limit, ymax = Aggregate_High_Confidence_Limit), width = 0.2) +
#       geom_point() +
#       labs(title = "Minnesota State", x = "Data Value Type", y = "Estimate (Rate)") +
#       theme_minimal()
#   })
# }
# 
...
# calc_aggregate_values <- function(df, userInput, filterBy) {
#   df |>
#     filter("filterBy" == userInput)
#   mutate(Aggregate_Data_Value = Data_Value*population/100, # Use Data_Value directly as rates
#          Aggregate_Low_Confidence_Limit = Low_Confidence_Limit*population/100,
#          Aggregate_High_Confidence_Limit = High_Confidence_Limit*population/100) |> # Keep confidence limits as rates
#     group_by (CHB data_value_type) |>
#     mutate(aggregateEstimate = sum(aggregate_data_value) / sum(population) *100,
#            Aggregate_LowEstimate = sum(Aggregate_Low_Confidence_Limit) / sum(population) *100,
#            Aggregate_HighEstimate = sum(Aggregate_High_Confidence_Limit) / sum(population) *100
#     )
# }
